...
def active_features() -> list[str]:
    ...
    for feature, specs in LAZY_DEPS.items():
        if feature == "platform.matrix_e2ee":
            # E2EE is an opt-in dependency bundle rather than a backend with
            # one authoritative anchor.  When it is explicitly requested,
            # any installed member is evidence of a partial/previous install
            # that `hermes update` should repair and refresh.
            if not _matrix_e2ee_requested():
                continue
            present = any(_is_present(spec) for spec in specs)
        else:
            present = bool(specs and _is_present(specs[0]))
        if present:
