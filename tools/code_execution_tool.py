...
def execute_code(
    ...
    if _guard.get("user_approved"):
        ...
        # Preserve the existing cancellation contract for scripts that were
        # auto-approved rather than explicitly approved by the user.  Check
        # before sandbox setup/spawn so a short child cannot finish while the
        # parent is descheduled before its first poll.
        return json.dumps({
            "status": "interrupted",
            "output": _format_interrupted_output(""),
            "exit_code": 130,
            "tool_calls_made": 0,
            "duration_seconds": 0,
        }, ensure_ascii=False)
