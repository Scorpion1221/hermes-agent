...
def _load_local_whisper_model(model_name: str, device: str = "auto", compute_type: str = "auto"):
    ...
    if force_cpu:
        # The platform guard owns the device choice, but an explicit
        # CPU-safe compute type remains valid user configuration. Preserve it
        # instead of silently rewriting e.g. cpu/float32 to int8.
        safe_compute_type = (
            compute_type
            if str(device).lower() == "cpu" and str(compute_type).lower() != "auto"
            else "int8"
        )
        logger.info(
