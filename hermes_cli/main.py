...
def _select_update_branch(
    ...
    """Choose the maintenance branch without trapping feature branches.

    Explicit ``--branch`` always wins. Otherwise a simple current branch is
    retained when it exists on origin; feature branches and detached HEADs
    fall back to ``develop`` for this fork and ``main`` for the canonical repo.
    """
    ...
    if (
        preferred
        and "/" not in preferred
        and _remote_branch_exists(git_cmd, cwd, preferred)
    ):
        return preferred
...
def _resolve_update_branch(args) -> str:
    """Resolve explicit or checkout-aware update branch selection."""
    ...
    try:
        ...
        origin_url = _self()._get_origin_url(git_cmd, PROJECT_ROOT)
        return _select_update_branch(
            git_cmd,
            PROJECT_ROOT,
            current_branch or "HEAD",
            _self()._is_fork(origin_url),
        )
    except Exception:
        return OFFICIAL_STABLE_BRANCH
...
def _resolve_update_check_branch(args) -> str:
    """Keep ``hermes update --check`` aligned with the apply path."""
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        explicit_branch = getattr(args, "branch", None)
        pip_branch = bool(explicit_branch) and detect_install_method(PROJECT_ROOT) == "pip"
        if pip_branch:
            print(
                "⚠ --branch is ignored for PyPI installs "
                f"(would have checked '{explicit_branch}')."
            )
        _self()._cmd_update_check(
            branch=OFFICIAL_STABLE_BRANCH if pip_branch else _resolve_update_check_branch(args),
            branch_explicit=bool(explicit_branch),
        )
        return
