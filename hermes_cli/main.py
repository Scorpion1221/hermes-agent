...
def _select_update_branch(
    ...
    Policy:
    - An explicit ``--branch`` (explicit_branch) always wins — upstream's
      ``--branch`` contract must hold even on the fork.
    - Otherwise keep updating the current simple maintenance branch when it
      exists on origin (e.g. main/develop).
    - Feature branches (``foo/bar``) fall back to the repo's stable branch.
    - For Scorpion1221 fork: develop
    - For official repo: main
    - Final fallback: main
    """
    # Explicit --branch overrides the fork's auto-tracking policy.
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Fetch and pull
    try:
        # Get current branch first so fork checkouts can keep tracking their
        # maintained branch (`develop`) while still benefiting from upstream's
        # scoped fetch optimization.
        result = subprocess.run(
            git_cmd + ["rev-parse", "--abbrev-ref", "HEAD"],
            cwd=PROJECT_ROOT,
            capture_output=True,
        ...
        # the stable branch remains `main`. An explicit --branch still wins.
        branch = _select_update_branch(
            git_cmd, PROJECT_ROOT, current_branch, is_fork,
            explicit_branch=getattr(args, "branch", None),
        )
        ...
        # official behavior; on the fork the default target is "develop".
