...
def _launchd_candidate_domains() -> tuple[str, ...]:
    """Return launchd domains to try, in preference order.

    macOS 26 made ``user/<uid>`` the only manageable domain for some
    non-Aqua/background sessions, but existing login-window agents can still be
    loaded under ``gui/<uid>``.  Treat ``user`` as the preferred modern target
    while falling back to ``gui`` before degrading to a detached process.
    """

    domains: list[str] = []
    for domain in (_launchd_domain(), f"user/{os.getuid() if hasattr(os, 'getuid') else 0}", _launchd_gui_domain()):
        if domain not in domains:
            domains.append(domain)
    return tuple(domains)


def _launchd_target(domain: str, label: str) -> str:
    return f"{domain}/{label}"


def _launchctl_called_process_error(
...
def _run_launchctl_quiet(cmd: list[str], *, timeout: int = 30) -> subprocess.CompletedProcess:
    """Run launchctl without leaking expected domain-probe errors to stdout."""
...
def _launchd_bootout(label: str, plist_path: Path | None = None) -> None:
    """Unload the service from any launchd domain that may hold it.

    Prefer the domain + plist-path form.  In practice it can remove stale
    in-memory jobs that ``bootout domain/label`` reports as "No such process",
    which otherwise makes the next ``bootstrap`` fail with launchctl exit 5.
    """

    for domain in _launchd_candidate_domains():
        if plist_path is not None and plist_path.exists():
            try:
                result = _run_launchctl_quiet(
        ...
        try:
            _run_launchctl_quiet(
                ["launchctl", "bootout", _launchd_target(domain, label)],
                timeout=90,
            )
...
def _launchd_bootstrap(plist_path: Path) -> str:
    """Bootstrap the plist, recovering stale labels and trying alternate domains."""
    ...
    if last_error is not None:
        raise last_error
    raise subprocess.CalledProcessError(1, ["launchctl", "bootstrap", str(plist_path)])


def _launchd_kickstart_with_fallback(label: str, *, force: bool = False) -> str:
    """Kickstart the service in whichever candidate domain currently owns it."""

    last_error: subprocess.CalledProcessError | None = None
    for domain in _launchd_candidate_domains():
        target = _launchd_target(domain, label)
        cmd = ["launchctl", "kickstart"]
        if force:
            cmd.append("-k")
        cmd.append(target)
        try:
            result = _run_launchctl_quiet(cmd, timeout=90 if force else 30)
        except subprocess.CalledProcessError as exc:
            if not _launchd_error_indicates_unloaded(
                exc
            ) and not _launchctl_domain_unsupported(exc.returncode):
                raise
            last_error = exc
            continue
        if result.returncode == 0:
        ...
        exc = _launchctl_called_process_error(result, cmd)
        if not _launchd_error_indicates_unloaded(exc) and not _launchctl_domain_unsupported(
            exc.returncode
        ):
            raise exc
        last_error = exc
    ...
    if last_error is not None:
        raise last_error
    raise subprocess.CalledProcessError(1, ["launchctl", "kickstart", label])
...
def _retry_launchctl_bootstrap_until_registered(
    domain: str | tuple[str, ...], plist_path, label: str, *, deadline: float
) -> bool:
    ...
    while True:
        ...
        for candidate in domains:
            try:
                _launchctl_bootstrap(candidate, plist_path, label, timeout=30)
                if _launchctl_label_supervising_process(label):
                    return True
                _append_launchd_reload_log(
                    f"bootstrap attempt {attempt} exited 0 but {candidate}/{label} "
                    f"has no supervised process (launchctl list) — retrying"
                )
            except subprocess.CalledProcessError as exc:
                _append_launchd_reload_log(
                    f"bootstrap attempt {attempt} failed (rc={exc.returncode}) "
                    f"for {candidate}/{label} — retrying"
                )
            except subprocess.TimeoutExpired:
                _append_launchd_reload_log(
                    f"bootstrap attempt {attempt} timed out for {candidate}/{label} "
                    f"— retrying"
                )
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # Retry until launchctl lists the label, trying every viable domain on
        # each pass. A one-shot bootstrap can lose a race under high load and
        # leave the gateway permanently outside launchd supervision.
        ...
        candidate_domains = _launchd_candidate_domains()
        bootout_cmds = []
        bootstrap_cmds = []
        for domain in candidate_domains:
            bootout_cmds.append(
                "launchctl bootout "
                f"{shlex.quote(domain)} {shlex.quote(str(plist_path))} 2>/dev/null || "
                f"launchctl bootout {shlex.quote(_launchd_target(domain, label))} 2>/dev/null || true"
            )
            bootstrap_cmds.append(
                "launchctl bootstrap "
                f"{shlex.quote(domain)} {shlex.quote(str(plist_path))} 2>/dev/null; "
                f"if launchctl list {shlex.quote(label)} 2>/dev/null | "
                "grep -qE '\"PID\" = [0-9]+;'; then break; fi"
            )
        targets = ", ".join(_launchd_target(domain, label) for domain in candidate_domains)
        ...
        _append_launchd_reload_log(f"Launchd reload helper started for {targets}")
        ...
        reload_script = (
            ...
            + "; ".join(bootstrap_cmds)
            + "; "
            f"  echo \"[$(date '+%Y-%m-%d %H:%M:%S %z')] bootstrap has no supervised process for {targets} — retrying\" >> {shlex.quote(str(reload_log_path))}; "
            f"  if [ $(date +%s) -ge $_deadline ]; then break; fi; "
            f"  sleep 2; "
            f"done; "
            f"if ! launchctl list {shlex.quote(label)} 2>/dev/null | grep -qE '\\\"PID\\\" = [0-9]+;'; then "
            f"  echo \"[$(date '+%Y-%m-%d %H:%M:%S %z')] FAILED launchd reload for {targets} — service NOT registered after {_reload_budget}s of retries\" >> {shlex.quote(str(reload_log_path))}; "
            f"fi; "
        ...
        try:
            ...
            _append_launchd_reload_log(
                f"FAILED to spawn launchd reload helper for {targets}: {e} — "
    ...
    # Boot out every possible registration, then retry bootstrap across all
    # viable domains until launchctl confirms the label is registered.
    candidate_domains = _launchd_candidate_domains()
    targets = ", ".join(
        _launchd_target(domain, label) for domain in candidate_domains
    )
    _launchd_bootout(label, plist_path)
    ...
    if gateway_pid is not None and not _wait_for_pid_exit(
        ...
        _append_launchd_reload_log(
            ...
            f"{int(_reload_budget)}s drain wait — bootstrapping {targets} anyway"
        )
    _deadline = time.monotonic() + _reload_budget
    if not _retry_launchctl_bootstrap_until_registered(
        candidate_domains, plist_path, label, deadline=_deadline
    ):
        targets = ", ".join(
            _launchd_target(domain, label) for domain in candidate_domains
        )
        _append_launchd_reload_log(
            f"FAILED launchd reload of {targets} — service NOT registered after "
...
def launchd_start():
    ...
    if not plist_path.exists():
        ...
        try:
            _launchd_bootstrap(plist_path)
            _launchd_kickstart_with_fallback(label)
        except subprocess.CalledProcessError as e:
            if not _launchctl_domain_unsupported(e.returncode):
                raise
    ...
    try:
        ...
        if not _launchd_error_indicates_unloaded(e) and not _launchctl_domain_unsupported(
            e.returncode
        ):
            raise
...
def launchd_restart():
    ...
    try:
        ...
        _launchd_kickstart_with_fallback(label, force=True)
