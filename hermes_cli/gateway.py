    ...
    if (
        ...
        for domain in candidate_domains:
            ...
            bootstrap_cmds.append(
                "launchctl bootstrap "
                f"{shlex.quote(domain)} {shlex.quote(str(plist_path))} 2>/dev/null; "
                f"if launchctl list {shlex.quote(label)} >/dev/null 2>&1; then exit 0; fi"
            )
        ...
        reload_script = (
            ...
            + "; ".join(bootstrap_cmds)
            + "; "
            f"  echo \"[$(date '+%Y-%m-%d %H:%M:%S %z')] bootstrap not yet registered for {targets} — retrying\" >> {shlex.quote(str(reload_log_path))}; "
