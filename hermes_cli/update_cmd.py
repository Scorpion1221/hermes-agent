...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    # `git status` reports an untracked nested repository, but `git stash`
    # cannot capture it. Depending on git version it may create an empty stash
    # commit anyway, which would trigger a misleading restore prompt. Preserve
    # the upstream partial-stash handling above, then discard only a newly
    # created entry that contains no tracked or untracked paths.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    def _dirty(*extra):
        # Do not trust Git's cached stat match here.  A large checkout can
        # finish within the filesystem timestamp granularity, leaving some
        # freshly-written CRLF files looking unchanged until a later command.
        # This repair runs only for legacy autocrlf=true installs, so paying
        # for one forced index refresh is preferable to silently missing a
        # subset of the churn and persisting a broken checkout.
        subprocess.run(
        ...
        # ``--name-only --ignore-cr-at-eol`` still reports CRLF-only files
        # even though the rendered diff is empty. ``--numstat`` applies the
        # whitespace filter before emitting records, so it can distinguish
        # pure EOL churn from real edits in one process. Disable rename
        # detection to keep every NUL-delimited record in the simple
        # ``adds<TAB>deletes<TAB>path`` form.
        out = subprocess.run(
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(f"  curl -fsSL {_m().FORK_INSTALL_URL} | bash")
            sys.exit(1)
