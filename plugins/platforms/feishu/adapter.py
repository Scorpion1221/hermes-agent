...
# Catches the most common Feishu error wordings when an `interactive` card
# payload is rejected (invalid schema, malformed card content, etc.). When
# this matches, send/edit fall back to plain text rather than retrying.
_CARD_CONTENT_INVALID_RE = re.compile(
    r"(invalid.{0,40}(card|interactive|schema)|"
    r"(card|interactive|schema).{0,40}invalid|"
    r"card.{0,40}(format|content|schema))",
    re.IGNORECASE,
)
# ---------------------------------------------------------------------------
...
def _build_card_v2_payload(content: str) -> str:
    """Build a Feishu Card 2.0 message with a single markdown element.

    Card 2.0's `markdown` element supports the full standard Markdown set
...
def parse_feishu_post_payload(
    ...
    *,
    ...
    title = _normalize_feishu_text(str(resolved.get("title", "")).strip(), mentions_map)
    if title:
        parts.append(title)
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            # Fork: default group policy to "open" (c0b13e9fc), not upstream's
            # "allowlist" — feishu groups are open unless explicitly restricted.
        ...
        try:
            for chunk in chunks:
                ...
                try:
                    ...
                    elif msg_type == "interactive" and _CARD_CONTENT_INVALID_RE.search(str(exc)):
                        logger.warning("[Feishu] Card payload rejected by API; falling back to plain text")
                        response = await self._feishu_send_with_retry(
                            chat_id=chat_id,
                            msg_type="text",
                            payload=json.dumps({"text": _strip_markdown_to_plain_text(chunk)}, ensure_ascii=False),
        ...
        try:
            ...
            should_fallback = (
                ...
                and (
                    ...
                    or (msg_type == "interactive" and _CARD_CONTENT_INVALID_RE.search(result.error or ""))
                )
            )
            if should_fallback:
                logger.warning(
                    "[Feishu] Invalid %s update payload rejected by API; falling back to plain text",
                    msg_type,
                )
    ...
    async def stop_all_streaming_cards(self) -> None:
        ...
        for message_id, state in to_stop:
            ...
            logger.info(
                "[Feishu] Force-stopped streaming card %s (content=%d chars, closed=%s, updated=%s)",
                state.card_id,
                len(content),
                streaming_closed,
                final_card_updated,
    ...
    def _on_drive_comment_event(self, data: Any) -> None:
        """Schedule drive document comment notifications on the adapter loop."""
        loop = self._loop
        if not self._loop_accepts_callbacks(loop):
            logger.warning("[Feishu] Dropping drive comment event before adapter loop is ready")
            return
        future = asyncio.run_coroutine_threadsafe(
            self._handle_drive_comment_event_data(data),
            loop,
        )
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        # Fork: richer name resolution (cache hit, else brief contact-API
        # lookup) so the "Approved by …" card shows a real name.
    ...
    async def _extract_message_content(
        ...
        """Extract text, cached media, and mention metadata from a Feishu message."""
        ...
        mention_refs = list(
            _build_mentions_map(getattr(message, "mentions", None), self._bot_identity()).values()
        )
        ...
        mention_refs = list(normalized.mentions or mention_refs)
        seen_mention_ids = {ref.open_id for ref in mention_refs if ref.open_id}
        for mentioned_id in getattr(message_context, "mentioned_ids", ()) or ():
            mentioned_id = str(mentioned_id or "").strip()
            if (
                not mentioned_id
                or mentioned_id in seen_mention_ids
                or mentioned_id.startswith("@_user_")
                or mentioned_id == "@_all"
            ):
                continue
            mention_refs.append(
                FeishuMentionRef(
                    open_id=mentioned_id,
                    is_self=self._bot_identity().matches(open_id=mentioned_id, user_id="", name=""),
                )
            )
            seen_mention_ids.add(mentioned_id)
        ...
        text = inbound_content.text
        if not injected and message_context.content_type in {"text", "post"} and normalized.text_content:
    ...
    async def _resolve_sender_name_from_api(
        ...
        *,
        ...
        if is_bot:
            return await self._resolve_bot_name_from_api(sender_id)
    ...
    async def _resolve_bot_name_from_api(self, sender_id: Optional[str]) -> Optional[str]:
        """Resolve a Feishu bot's display name via /bot/v3/bots/basic_batch.

        Falls through to the same FeishuSenderNameCache as humans so subsequent
        lookups are deduped, but bypasses the contact API which never returns
        bot names.
        """
        if not sender_id:
            return None
        ...
        if not trimmed:
            return None
        # Cache hit (positive or negative "" entry). Tolerate plain-dict mocks
        # that older tests inject in place of FeishuSenderNameCache.
        cache = self._sender_name_cache
        cached_entry = cache.get(trimmed)
        if cached_entry is not None:
            # FeishuSenderNameCache.get returns (name, expires_at); mocks may
            # return a tuple, an entry-like object, or just a string.
            if isinstance(cached_entry, tuple) and cached_entry:
                cached_name = cached_entry[0]
            else:
                cached_name = getattr(cached_entry, "user_name", cached_entry)
            return (cached_name or None) if isinstance(cached_name, str) else None
        names = await self._fetch_bot_names([trimmed])
        if names is None:
            # API failure — do not poison cache so a later retry can succeed.
            return None
        # API success: cache every returned id (including "" for known-nameless).
        # Skip ids absent from response → no cache entry, next lookup re-fetches.
        for oid, name in names.items():
            self._cache_bot_name(oid, name)
        # Re-read for the trimmed id; tolerate plain-dict mocks.
        hit = cache.get(trimmed)
        if hit is None:
            return None
        if isinstance(hit, tuple) and hit:
            return hit[0] or None
        name_attr = getattr(hit, "user_name", hit)
        return (name_attr or None) if isinstance(name_attr, str) else None

    def _cache_bot_name(self, sender_id: str, name: str) -> None:
        """Insert a bot-name cache entry, supporting empty (negative) entries.

        Tolerates plain-dict mocks that older tests inject in place of
        FeishuSenderNameCache.
        """
        if not sender_id:
            return
        cache = self._sender_name_cache
        # Plain dict mock — fall back to ``cache[id] = (name, expires_at)``.
        if not hasattr(cache, "_entries"):
            try:
                ttl = getattr(cache, "_ttl_seconds", _FEISHU_SENDER_NAME_TTL_SECONDS)
                cache[sender_id] = (name or "", time.time() + ttl)
            except Exception:
                pass
            return
        ...
        normalized_id = normalize_feishu_sender_id(sender_id)
        if not normalized_id:
            return
        ttl = getattr(cache, "_ttl_seconds", _FEISHU_SENDER_NAME_TTL_SECONDS)
        time_fn = getattr(cache, "_time_fn", time.time)
        now = time_fn()
        app_id = getattr(self, "_app_id", None)
        entry = FeishuSenderNameCacheEntry(
            account_scope=normalize_feishu_account_scope(app_id or None),
            sender_id=normalized_id,
            user_name=name or "",
            cached_at=now,
            expires_at=now + ttl,
        )
        cache._entries[(entry.account_scope, entry.sender_id)] = entry
    ...
    async def _batch_resolve_sender_names_from_api(self, sender_ids: List[str] | tuple[str, ...]) -> Dict[str, Optional[str]]:
        ...
        try:
            ...
            response = await self._run_blocking(self._client.contact.v3.user.batch, request)
            if not response or not response.success():
                logger.info(
        ...
        try:
            ...
            if hasattr(self._client, "request"):
                try:
                    ...
                    raw_response = await self._run_blocking(self._client.request, raw_request)
    ...
    async def _fetch_message_text(self, message_id: str) -> Optional[str]:
        if not hasattr(self, "_quoted_context_cache") or not hasattr(self, "_message_items_cache"):
            if not self._client or not message_id:
                return None
            cached_text = getattr(self, "_message_text_cache", {}).get(message_id)
            ...
            try:
                ...
                if hasattr(self, "_message_text_cache"):
    ...
    def _build_outbound_payload(
        ...
        # All outbound messages use Feishu Card 2.0 so markdown (tables,
        # headings, lists, code) renders consistently. Plain text fallback
        # is handled by send/edit_message error paths if the API rejects
        # the card payload. ``prefer_post`` is intentionally accepted but
        # ignored: the fork's Card 2.0 transport already keeps every chunk of
        # a long markdown response on one consistent rendering path.
        return "interactive", _build_card_v2_payload(content)
    ...
    try:
        ...
        if thread_id:
            ...
            try:
                ...
                # Feishu topic replies require a message anchor. Only attach it
                # when the active session belongs to the same topic; otherwise
                # the reply API can graft the message into a foreign topic.
                if anchor and session_thread_id and session_thread_id == str(thread_id):
                    metadata["reply_to_message_id"] = anchor
            except Exception:
                logger.debug("[Feishu] Could not resolve session reply anchor", exc_info=True)
