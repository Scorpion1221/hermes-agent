...
describe.skipIf(onWindows)('execFileNoThrow with daemon-style children', () => {
  ...
  it("settles immediately on 'exit' when resolveOnExit is true, regardless of daemon stdio", async () => {
    ...
    // Completion must precede the daemon's exit, independent of CPU load.
    // An elapsed-time threshold measured scheduler speed instead of this
    // relationship and failed under otherwise healthy parallel CI runs.
