...
class TestNodeToolRunnable:
    ...
    def test_probe_exit_status_controls_runnable(self, tmp_path, monkeypatch):
        npm = self._stub(tmp_path, "npm", "#!/bin/sh\nexit 0\n")
        outcomes = iter((SimpleNamespace(returncode=0), SimpleNamespace(returncode=1)))
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hermes_constants, "_managed_node_heal_attempted", False)
        # The logic under test is the broken -> heal -> re-probe transition.
        # Avoid executing a temp script while its inode is rewritten: macOS
        # endpoint-security scanners can intermittently hold that launch for
        # the entire 10s probe timeout under the parallel suite.
        monkeypatch.setattr(
            hermes_constants,
            "node_tool_runnable",
            lambda path: "exit 0" in Path(path).read_text(),
        )
    ...
    def test_broken_managed_npm_returns_none_when_heal_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hermes_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(hermes_constants, "heal_hermes_managed_node", lambda: False)
        monkeypatch.setattr(hermes_constants, "node_tool_runnable", lambda path: False)
