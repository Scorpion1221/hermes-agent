...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_template_with_spaces_is_shell_quoted(self):
        """The static path portion must be shlex-quoted (Windows/Git-Bash
        ``C:/Users/...`` or spaces) before it is passed to ``mktemp``."""
        env = _TestableEnv()
        env._snapshot_ready = True
    ...
    def test_wrap_command_mv_chained_on_export_success(self):
        ...
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
        # Upstream now unsets per-session variables in a subshell before
        # dumping, so the redirect belongs to the enclosing brace group rather
        # than directly to `export -p`.  Preserve the same atomic invariant:
        # only a successful complete group may reach `mv`.
        assert "export -p;" in wrapped
        assert '> "$__hermes_snap_tmp" && mv -f ' in wrapped
