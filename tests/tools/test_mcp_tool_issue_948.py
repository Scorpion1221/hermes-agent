    ...
    async def _test():
        with patch("tools.osv_check.check_package_for_malware", side_effect=hung_check), \
            ...
            try:
                ...
                release_check.set()
                await server.shutdown()
        # A permanently blocked check cannot hold startup past the bounded
        # preflight timeout. The wider outer ceiling remains stable under the
        # repository's 28-process parallel runner.
        assert elapsed < 3, f"startup did not fail-open promptly ({elapsed:.1f}s)"
