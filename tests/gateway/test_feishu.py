...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_download_feishu_image_uses_persistent_media_index_hit(self):
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
        from gateway.platforms.feishu_inbound import FeishuMediaIndexEntry
        ...
        with patch("plugins.platforms.feishu.adapter.get_feishu_media_index_entry", return_value=cached) as lookup:
    ...
    def test_download_feishu_message_resource_writes_persistent_media_index(self):
        ...
        with (
            ...
            patch("plugins.platforms.feishu.adapter.cache_image_from_bytes", return_value="/tmp/cached-quoted.png"),
            patch("plugins.platforms.feishu.adapter.put_feishu_media_index_entry") as store,
        ):
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_send_uses_interactive_card_for_every_markdown_chunk(self):
        """Every chunk stays on the fork's Card 2.0 markdown path."""
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
        ...
        self.assertEqual(
            [request.request_body.msg_type for request in captured],
            ["interactive", "interactive"],
        )

    @patch.dict(os.environ, {}, clear=True)
    def test_send_plain_text_message_uses_interactive_card(self):
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
        ...
        class _MessageAPI:
            def create(self, request):
                ...
                return SimpleNamespace(
                    success=lambda: True,
                    data=SimpleNamespace(message_id="om_plain_card"),
                )
        ...
        self.assertEqual(captured[0].request_body.msg_type, "interactive")
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_build_post_payload_splits_fenced_code_blocks_into_separate_rows(self):
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
    ...
    def test_send_uses_post_for_every_chunk_of_multi_chunk_markdown(self):
        ...
        across multiple chunks, every chunk must use the same rich-Markdown
        surface — including chunk 1.  The fork's Card 2.0 renderer supersedes
        the legacy ``post`` surface while preserving the upstream invariant:
        a plain-looking first chunk must not fall back to ``text`` and make
        later Markdown render inconsistently.
        """
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
        ...
        markdown_chunks = [
            json.loads(r.request_body.content)["body"]["elements"][0]["content"]
            for r in captured
        ]
        # Card 2.0 normalizes H1-H3 headings to H4 for consistent rendering.
        self.assertEqual(
            markdown_chunks,
            [first_chunk, "#### Heading\nAnd then some **bold** text."],
        )
