...
class TestRunBackgroundTask:
    ...
    async def test_media_files_routed_by_type(self, monkeypatch):
        ...
        try:
            ...
            _real_ogg = _os.path.realpath(_ogg)
            _real_mp4 = _os.path.realpath(_mp4)
            _real_png = _os.path.realpath(_png)
            _real_pdf = _os.path.realpath(_pdf)

            mock_adapter.send_voice.assert_called_once()
            assert mock_adapter.send_voice.call_args.kwargs["audio_path"] == _real_ogg
            mock_adapter.send_video.assert_called_once()
            assert mock_adapter.send_video.call_args.kwargs["video_path"] == _real_mp4
            mock_adapter.send_image_file.assert_called_once()
            assert mock_adapter.send_image_file.call_args.kwargs["image_path"] == _real_png
            mock_adapter.send_document.assert_called_once()
            assert mock_adapter.send_document.call_args.kwargs["file_path"] == _real_pdf
        finally:
