...
@pytest.mark.asyncio
async def test_run_agent_persists_canonical_quoted_message_without_duplicate_prefix(monkeypatch, tmp_path):
    _install_fake_agent(monkeypatch)
    runner = _make_runner()
    ...
    assert result["final_response"] == "ok"
    assert _CapturingAgent.last_run["persist_user_message"] == canonical_message
