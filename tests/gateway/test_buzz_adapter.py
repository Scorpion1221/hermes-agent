...
class TestInboundMediaAuthorizationGate:
    ...
    async def test_live_media_redacts_long_path_before_bounding(self, tmp_path):
        parent = tmp_path
        private_parts = []
        # Exceed the 900-character error bound, not macOS PATH_MAX (1024).
        part_length = (930 - len(str(tmp_path))) // 6
        for index in range(6):
            prefix = f"private-{index}-"
            part = prefix + ("x" * (part_length - len(prefix)))
