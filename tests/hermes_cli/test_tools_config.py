...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    with (
        mock_patch("hermes_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}),
        mock_patch("hermes_cli.tools_config.CONFIGURABLE_TOOLSETS", configurable_toolsets),
        mock_patch("hermes_cli.tools_config._DEFAULT_OFF_TOOLSETS", set()),
        mock_patch("hermes_cli.tools_config._TOOLSET_PLATFORM_RESTRICTIONS", {}),
        mock_patch("toolsets.TOOLSETS", fake_toolsets),
    ):
        enabled = _get_platform_tools({}, "_test_platform")

    # Configurable toolset recovery is covered elsewhere; this test's
    # regression target is the non-configurable platform-specific toolset.
