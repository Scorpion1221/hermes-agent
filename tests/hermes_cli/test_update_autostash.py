...
def _patch_managed_uv(request, monkeypatch):
    ...
    # Updater unit tests must never discover or restart the host gateway fleet.
    ...
    # These tests fake git pull: no on-disk runtime changed. Reload/purge
    # would discard the fixtures and reconnect to real services/config.
    monkeypatch.setattr("hermes_cli.update_cmd._purge_stale_hermes_modules", lambda: None)
    monkeypatch.setattr("hermes_cli.update_cmd._reload_config_modules", lambda: None)
...
def test_restore_stashed_changes_prompts_before_applying(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(hermes_main.subprocess, "run", fake_run)
    monkeypatch.setattr("hermes_cli.update_cmd._critical_module_import_failures", lambda *a, **kw: {})
    monkeypatch.setattr("builtins.input", lambda: "")

    restored = hermes_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)

    calls = [(cmd, kw) for cmd, kw in calls if cmd[1:2] != ["ls-files"] and "-z" not in cmd]
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    (tmp_path / "venv").mkdir()  # Model the managed install, not a missing venv.
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(
    ...
    """A broken optional extra must not block base deps or healthy extras."""
...
def test_cmd_update_succeeds_with_extras(monkeypatch, tmp_path):
    """A successful aggregate extras install needs no fallback."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr(
        "shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None
    )
...
def test_install_with_optional_fallback_honors_custom_group(monkeypatch):
    """Termux update path must target its own optional dependency group."""
    calls = []
    monkeypatch.setattr(
        hermes_main,
        "_load_installable_optional_extras",
...
def test_install_heartbeat_prints_when_dependency_install_is_silent(
    ...
    hermes_main._run_install_with_heartbeat(
        ["uv", "pip", "install", "-e", "."], heartbeat_interval_seconds=1
    )
    assert "still installing dependencies" in capsys.readouterr().out
...
def test_cmd_update_falls_back_to_reset_when_ff_only_fails(
    ...
    """Diverged managed history is repaired from the selected remote branch."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr(
        "shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None
    )
...
def test_cmd_update_no_reset_when_ff_only_succeeds(monkeypatch, tmp_path):
    """A normal fast-forward must never discard the checkout."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr(
        "shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None
    )
...
def test_cmd_update_restores_stash_and_keeps_main_after_merged_branch_repair(monkeypatch, tmp_path, capsys):
    """A fully merged parked branch is repaired to main without losing its stash."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    # The fake ancestry probe reports the branch fully merged. Do not undo
    # the upstream parked-branch repair by checking it back out.
    assert ["git", "checkout", "main"] in recorded
    assert not [c for c in recorded if "checkout" in c and "fix/something" in c]
...
def test_cmd_update_network_error_shows_friendly_message(
    monkeypatch, tmp_path, capsys
):
    _setup_update_mocks(monkeypatch, tmp_path)
...
def test_non_interactive_defaults_to_stash_when_setting_absent(
    ...
    monkeypatch.setattr(
        hermes_config, "load_config", lambda *a, **kw: {"model": {}}
    )
    hermes_main.cmd_update(SimpleNamespace(gateway=True))
    assert len(restore_calls) == 1
    assert not discard_calls
