...
def _dirty(repo: Path) -> set[str]:
    # Force Git to re-hash the worktree instead of accepting a racy stat-cache
    # match when thousands of files were checked out in the same timestamp
    # tick.  The updater performs the same refresh before classifying churn.
    subprocess.run(
