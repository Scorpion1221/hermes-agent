...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        candidate_domains = ("user/501", "gui/501")
        monkeypatch.setattr(
            gateway_cli,
            "_launchd_candidate_domains",
            lambda: candidate_domains,
        )
        monkeypatch.setattr(
    ...
    def test_refresh_falls_back_to_direct_reload_when_helper_cannot_spawn(
        ...
        assert service_calls[:3] == [
            ["launchctl", "bootout", "user/501", str(plist_path)],
    ...
    def test_launchd_restart_boots_out_stale_registration_before_bootstrap(
        ...
        monkeypatch.setattr(gateway_cli, "probe_gateway_loop_liveness", lambda pid: "alive")
    ...
    def test_launchd_restart_drains_running_gateway_before_kickstart(self, monkeypatch, capsys):
        calls = []
        target = f"user/501/{gateway_cli.get_launchd_label()}"
        monkeypatch.setattr(gateway_cli, "_launchd_candidate_domains", lambda: ("user/501",))

        monkeypatch.setattr(gateway_cli, "refresh_launchd_plist_if_needed", lambda: False)
        monkeypatch.setattr(gateway_cli, "_get_restart_drain_timeout", lambda: 12.0)
        monkeypatch.setattr(gateway_cli, "_request_gateway_self_restart", lambda pid: False)
        monkeypatch.setattr(gateway_cli, "probe_gateway_loop_liveness", lambda pid: "alive")
        monkeypatch.setattr(gateway_cli, "_graceful_restart_via_sigusr1", lambda pid, timeout: calls.append(("sigusr1", pid, timeout)) or False)
        monkeypatch.setattr(gateway_cli, "_get_restart_exit_wait_budget", lambda: 12.0)
        monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "user/501")
...
class TestGatewaySystemServiceRouting:
    ...
    def _systemd_preflight_ready(self, monkeypatch):
        """These routing tests mock systemctl and do not need host systemd."""
        monkeypatch.setattr(
            gateway_cli,
...
@pytest.mark.macos_only
def test_launchd_restart_preserves_supervised_replacement(monkeypatch):
    calls = []
    monkeypatch.setattr(gateway_cli, "refresh_launchd_plist_if_needed", lambda: calls.append("refresh") or False)
    monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "user/501")
    monkeypatch.setattr("gateway.status.get_running_pid", lambda: 321)
    monkeypatch.setattr(gateway_cli, "_request_gateway_self_restart", lambda pid: False)
    monkeypatch.setattr(gateway_cli, "probe_gateway_loop_liveness", lambda pid: "alive")
    monkeypatch.setattr(gateway_cli, "_get_restart_exit_wait_budget", lambda: 12.0)
    monkeypatch.setattr(gateway_cli, "_graceful_restart_via_sigusr1", lambda pid, budget: calls.append(("graceful", pid, budget)) or True)
    monkeypatch.setattr(gateway_cli, "_wait_for_launchd_service_pid", lambda label, pid, **kw: calls.append(("replacement", pid, kw["domain"])) or True)
    with patch.object(gateway_cli, "_launchd_kickstart_with_fallback") as kick:
        gateway_cli.launchd_restart()
    assert calls == ["refresh", ("graceful", 321, 12.0), ("replacement", 321, "user/501")]
