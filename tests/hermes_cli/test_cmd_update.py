...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_falls_back_to_main_when_branch_not_on_remote(
        ...
        # Reuse the scoped fetch and fast-forward locally instead of paying
        # for a second network pull.
    ...
    def test_update_uses_current_branch_when_on_remote(
        ...
        merge_cmds = [c for c in commands if "merge --ff-only" in c]
        assert len(merge_cmds) == 1
        assert "origin/develop" in merge_cmds[0]
...
class TestCmdUpdateCheckBranchFlag:
    ...
    @patch("hermes_cli.main._cmd_update_check")
    @patch("hermes_cli.config.detect_install_method", return_value="pip")
    def test_check_branch_warns_on_pypi_install(
        self, _mock_method, mock_check, capsys
    ):
        """Legacy PyPI installs must not silently pretend to honor --branch."""
        args = SimpleNamespace(check=True, branch="bb/gui")
