...
def test_guest_barriers_keep_platform_durability_when_unset(monkeypatch, tmp_path):
    _config(monkeypatch, {})
    conn = sqlite3.connect(tmp_path / "state.db")
    try:
        conn.execute("PRAGMA journal_mode=DELETE")
        conn.execute("PRAGMA synchronous=1")
        apply_durability_barriers(conn)
        # Darwin must keep the upstream FULL write barrier even without a
        # config override; other platforms preserve this connection setting.
